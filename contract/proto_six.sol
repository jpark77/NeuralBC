        ...
        while(i<eth_index){
            if(rest_tok > 0){ //exchange
                if( (eth_user_index[eth_users[i]][i]*ratio_eth/for_ratio) > rest_tok ){ //send token partly
                    ...
                    if (!Token(token).transfer(eth_users[i], rest_tok*for_fee1/for_fee2)) throw;
                    rest_tok = 0;
                    break;
                } else { //send token
                    ...
                    if (!Token(token).transfer(eth_users[i], amt_send*for_fee1/for_fee2)) throw;
                    i++;
                }
            } else {
                break;
        ...
        while(j<tok_index){
            if(rest_eth > 0){ //exchange
                if( (tok_user_index[tok_users[j]][j]*ratio_tok/for_ratio) > rest_eth ){ //send ether partly
                    ...
                    if (!tok_users[j].call.value(rest_eth*for_fee1/for_fee2)() ) throw;
                    rest_eth = 0;
                    break;
                } else { //send ether
                    ...
                    if (!tok_users[j].call.value(amt_send*for_fee1/for_fee2)()) throw;
                    j++;
                }
            } else {
                break;
